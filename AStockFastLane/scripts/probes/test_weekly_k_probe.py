...
def build_endpoint(symbol: str, weeks: int, adjust_type: str) -> str:
    kline_type = "week"
    fq_param = "" if adjust_type == "none" else adjust_type
    params = {
        "_var": "kline_week",
        "param": f"{symbol},{kline_type},,,{weeks},{fq_param}",
    }
    return f"{ENDPOINT_BASE}?{urlencode(params)}"


def fetch_tencent_weekly_k(symbol: str, weeks: int, adjust_type: str) -> tuple[dict[str, Any] | None, str | None]:
    endpoint = build_endpoint(symbol, weeks, adjust_type)
    request = Request(
        endpoint,
        headers={
            "User-Agent": (
...
def extract_bars(payload: dict[str, Any], symbol: str, adjust_type: str) -> tuple[list[dict[str, Any]], str | None]:
    ...
    raw_bars = None
    for key in ("week", "qfqweek", "hfqweek"):
    ...
    if not isinstance(raw_bars, list):
        return [], f"Unexpected response: no weekly k list found for adjust_type={adjust_type}"
...
def fetch_candidate(candidate: dict[str, Any], weeks: int, adjust_type: str, created_at: str) -> dict[str, Any]:
    code = candidate["code"]
    ...
    payload, fetch_error = fetch_tencent_weekly_k(symbol, weeks, adjust_type)
    if fetch_error:
    ...
    if not bars:
        return empty_item(candidate, market, "empty_bars", "No weekly K bars parsed from response", created_at, adjust_type)

    status = "ok"
    error_message = ""
    if len(bars) < 20:
        status = "insufficient_history"
        error_message = f"Only {len(bars)} weekly K bars parsed; minimum is 20"
...
def build_result(limit: int, weeks: int, source: str, adjust_type: str) -> dict[str, Any]:
    ...
    if source != "tencent":
        ...
        for candidate in candidates:
            items.append(fetch_candidate(candidate, weeks, adjust_type, created_at))
    ...
    return {
        "meta": {
            ...
            "adjust_type": adjust_type,
            "note": "low frequency weekly k data for MVP6 chart display",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch low-frequency weekly K data for candidate watchlist stocks.")
    parser.add_argument("--limit", type=int, default=DEFAULT_LIMIT, help="Maximum candidate count to fetch, default 10.")
    parser.add_argument("--weeks", type=int, default=DEFAULT_WEEKS, help="Weekly K bar count to request, default 80.")
    parser.add_argument("--source", default="tencent", choices=["tencent"], help="Weekly K source, default tencent.")
    parser.add_argument(
        "--adjust-type",
...
def main() -> int:
    ...
    result = build_result(limit=limit, weeks=weeks, source=args.source, adjust_type=args.adjust_type)

    today = now_local().strftime("%Y%m%d")
    dated_path = OUTPUT_DIR / f"weekly_k_{today}.json"
    write_json(LATEST_PATH, result)
    write_json(dated_path, result)

    meta = result["meta"]
    print("Weekly K probe: weekly_k")
    print(f"Source: {meta['source']}")
    print(f"Input: {meta['source_candidate_watchlist']}")
    print(f"Limit: {meta['limit']}")
    print(f"Weeks: {meta['weeks']}")
